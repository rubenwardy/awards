...
function awards.getFormspec(name, to, sid)
	...
	if #listofawards == 0 then
		...
		formspec = formspec .. "label[0,0;"..minetest.formspec_escape(S("Error: No awards available.")).."]"
		formspec = formspec .. "button_exit[1.5,1;3,1;close;"..minetest.formspec_escape(S("OK")).."]"
		return formspec
	end

	-- Sidebar
	if sid then
		formspec = formspec .. "size[11,5]"
